...
from sklearn.tree import DecisionTreeRegressor  # Import the specific estimator class

app = Flask(__name__)

# Load the trained machine learning model using the same estimator class
model = DecisionTreeRegressor()  # Create an instance of the estimator class
...
def predict():
    ...
    input_data = np.array([cb_flow, cb_press, cb_temp, steam_flow, steam_temp, steam_press, o2_press, o2_flow, o2_per, pci, atm_humid, hb_temp, hb_press, top_press, top_temp1, top_spray, top_temp, top_press_1, h2, skin_temp_avg, co, co2]).reshape(1, -1)
    ...
    return redirect(url_for('result', prediction_text=prediction_text, img_base64=img_base64, ratio_labels=ratio_labels, mse=mse, accuracy=accuracy))
...
def result():
    ...
    return render_template('result.html', prediction_text=prediction_text, img_base64=img_base64, ratio_labels=ratio_labels, mse=mse, accuracy=accuracy)
